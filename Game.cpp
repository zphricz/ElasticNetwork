...
Game::Game(SoftScreen *scr)
    : scr(scr), simulate(true), running(true),
      center_option(CenterType::AVERAGE_POSITION),
      scale_option(ScaleType::SCALE), time_step(0.4), steps_per_frame(10),
      selected_node(0) {
    srand(time(0));
    random_device rd;
    mt19937 gen(rd());
...
void Game::draw_network() {
    ...
    static int i = 0;
    if (i >= 10) {
        scr->fill_circle(scr->width / 2 + x, scr->height / 2 - y,
                         node_radius + 1, { 200, 200, 200 });
    }
    if (++i == 20) {
        i = 0;
    }
...
// Currently the selected node is not gauranteed to remiain the same over a
// single
